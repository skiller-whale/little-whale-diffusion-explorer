...
import { ddimStep, gaussianNoise, inferenceTimesteps, seedConditioning, timestepEmbedding } from "../src/diffusion";

ort.env.wasm.numThreads = 1;
const bytes = new Uint8Array(await Bun.file("public/models/whale-ddpm.onnx").arrayBuffer());
const session = await ort.InferenceSession.create(bytes, { executionProviders: ["wasm"] });
let sample = gaussianNoise(3 * 64 * 64, 24601);
const times = inferenceTimesteps(20);
const started = performance.now();
for (let index = 0; index < times.length; index++) {
  const timestep = times[index];
  const output = await session.run({
    sample: new ort.Tensor("float32", sample, [1, 3, 64, 64]),
    timestep_embedding: new ort.Tensor("float32", timestepEmbedding(timestep), [1, 192]),
    conditioning: new ort.Tensor("float32", seedConditioning(24601), [1, 16]),
  });
  if (!(output.predicted_velocity?.data instanceof Float32Array) || output.predicted_velocity.data.length !== 3 * 64 * 64) {
    throw new Error("Model output does not satisfy the browser contract");
  }
  sample = ddimStep(sample, output.predicted_velocity.data, timestep, times[index + 1] ?? -1);
}
